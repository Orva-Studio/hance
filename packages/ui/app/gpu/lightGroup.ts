...
// grain, vignette, film density) will actually run. The renderer only inserts
// the sRGB<->linear bracket when this holds, so a frame with the whole group
...
export function isLightGroupActive(params: PreviewParams): boolean {
  return (
    ...
    (params["no-film-density"] !== true && num(params, "film-density-amount", 0) > 0)
  );
}
