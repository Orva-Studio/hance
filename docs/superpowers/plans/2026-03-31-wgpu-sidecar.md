...
Expected: Builds successfully. Binary at `sidecar/target/release/openhancer-gpu`.
...
```json
{
  "scripts": {
    ...
    "build": "bun run build:gpu && bun run build:ui && bun build src/cli.ts --compile --outfile openhancer",
