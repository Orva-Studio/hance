...
**Architecture:** A standalone Rust binary (`openhancer-gpu`) communicates with Bun over stdin/stdout pipes. It receives raw RGBA frames, runs the same WGSL shader chain, and writes rendered RGBA frames back. The browser preview path is unchanged.
...
Implementation:
- `createHeadlessRenderer()` spawns `sidecar/target/release/openhancer-gpu` via `Bun.spawn`
...
**Binary location:** `sidecar/target/release/openhancer-gpu`
...
"build": "bun run build:gpu && bun build src/cli.ts --compile --outfile openhancer"
```
